  ...
  try {
    ...
    switch (action) {
      case "add": {
        ...
        const exists = userDraftsStorage[userId].some(
          (d) => d.key === draft.key
        );
        if (exists) {
          console.log(
            `⚠️ Draft ${draft.key} already exists for user ${userId}`
          );
          return NextResponse.json({
            success: true,
        ...
        console.log(
          `✅ Added draft ${draft.key} for user ${userId}. Total: ${userDraftsStorage[userId].length}`
        );
      ...
      case "remove": {
        ...
        userDraftsStorage[userId] = userDraftsStorage[userId].filter(
          (d) => d.key !== fileKey
        );
        ...
        console.log(
          `${removed ? "✅" : "⚠️"} ${
            removed ? "Removed" : "Tried to remove"
          } draft ${fileKey} for user ${userId}`
        );
