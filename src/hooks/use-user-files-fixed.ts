...
export function useUserFiles() {
  ...
  const updateFileThumbnail = useCallback(
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      const filesWithoutThumbnails = mergedFiles.filter(
        (file) =>
          !file.thumbnail_url && file.project_name !== "Archivos Manuales"
      );

      if (filesWithoutThumbnails.length > 0) {
        addLogEntry(
          `🖼️ Obteniendo vistas previas para ${filesWithoutThumbnails.length} archivos...`
        );
